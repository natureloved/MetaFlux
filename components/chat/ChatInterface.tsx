...
export default function ChatInterface({ mode = 'full' }: ChatProps) {
  ...
  const handleOpenChatWithAsset = (fqn: string, name: string) => {
    setActiveView('chat');
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', minWidth: 0 }}>
        ...
        <div className="scroll-area" style={{ flex: 1, overflow: 'hidden' }}>
          {activeView === 'chat' && (
          ...
          {activeView === 'explore' && (
            <ExploreView 
              ...
              onOpenChat={handleOpenChatWithAsset}
            />
          )}

          {activeView === 'lineage' && (
            <LineageView 
              targetFqn={lineageAsset}
              onBack={() => setActiveView('explore')}
              onSelectAsset={handleViewLineage}
            />
          )}
        </div>
